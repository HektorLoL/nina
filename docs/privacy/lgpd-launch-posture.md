...
| AI chat and Nina memory                                      | Consent                                                  | Server-recorded consent per adult in `nina_ai_consents`, enforced inside `begin_nina_chat_run` and `get_nina_weekly_candidates`; withdrawn grants are retained as a demonstrable record; revocation in Settings applies to the whole household immediately. |
