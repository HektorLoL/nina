...
alter table public.nina_proposals
  drop constraint if exists nina_proposals_kind_check;

alter table public.nina_proposals
  add constraint nina_proposals_kind_check
  check (kind in ('task', 'reminder', 'shopping', 'memory', 'seed'));

comment on column public.nina_proposals.kind is
  'What confirming this proposal creates. A semente is its own kind rather than a task with an empty date, because the closest thing Nina could otherwise offer for "um dia quero organizar o quartinho dos fundos" is an undated chore in Tarefas, and the one primitive that separates this product from a task list would stay reachable only by hand.';
