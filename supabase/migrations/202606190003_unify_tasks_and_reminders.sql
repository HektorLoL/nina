...
begin
  ...
  select
    case
      when existing_tasks.id is null then reminders.id
    ...
    reminders.detail,
    'Casa',
    reminders.date_label,
    reminders.due_at,
    'home',
    jsonb_build_object(
      'id', 'home',
      'title', 'Casa',
      'symbolName', reminders.symbol_name,
      'tone', reminders.tone
    ),
    'normal',
  ...
  left join public.tasks as existing_tasks on existing_tasks.id = reminders.id;

  get diagnostics migrated_count = row_count;

  if migrated_count <> reminder_count then
    raise exception 'reminder_task_migration_incomplete'
      using errcode = 'P0001';
  end if;
end;
$$;

create index if not exists tasks_family_due_at_idx
  on public.tasks(family_id, due_at)
  where due_at is not null and not is_done;

create index if not exists tasks_family_snoozed_until_idx
  on public.tasks(family_id, snoozed_until)
  where snoozed_until is not null and not is_done;
